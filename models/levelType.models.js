...
LevelType.getAll = function (queries, result) {
  ...
  connection.query(
    `SELECT * FROM levelType WHERE 1=1 ${query} ${order} ${limit}`,
    (err, res) => {
      if (err) {
...
LevelType.getAllBySectionId = function (id, queries, result) {
  ...
  connection.query(
    `SELECT * FROM levelType WHERE sectionId = ${id} ${query}`,
    (err, res) => {
      if (err) {
        console.log("Error while getting all LevelType", err);
        result(err, null);
...
LevelType.findById = function (id, result) {
  connection.query(
    `SELECT * FROM levelType WHERE idLevelType = ${id}`,
    (err, res) => {
      if (err) {
...
LevelType.delete = function (id, result) {
  connection.query(
    `DELETE FROM levelType WHERE idLevelType = ?`,
    id,
    (err, res) => {
      if (err) {
      ...
      result(null, {
        message: `LevelType ID ${id} has been deleted successfully`,
      });
    },
  );
};
