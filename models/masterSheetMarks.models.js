...
MasterSheetMarks.getAllForDocuments = function (collegeNumbers, result) {
  let fields = `'idMasterSheetMarks', idMasterSheetMarks, 'studentId', studentId, 'masterSheetMarkTypeId',masterSheetMarkTypeId ,'degree', degree, 'lessonId', lessonId, 'isFinal', isFinal, 'markStatusId', markStatusId, 'lessonName', lessonName, 'lessonCredit',lessonCredit, 'studyType', studyType, 'studyYearId', studyYearId, 'studyYear', (SELECT year FROM studentPortal.YearStudy WHERE studentPortal.YearStudy.idYearStudy = studyYearId), 'isCourses', (SELECT IF(levelType.masterSheetStudyTypeId = 1, false, true) FROM levelType WHERE levelType.level = masterSheet.studyLevel AND levelType.sectionId = masterSheet.sectionId AND levelType.studyYearId = masterSheet.studyYearId LIMIT 1), 'lessonCourse', (SELECT lessonCourse FROM lesson WHERE lesson.idLesson = lessonId), 'markTypeName', (SELECT markTypeName FROM markType WHERE idMarkType = masterSheetMarkTypeId)`;
