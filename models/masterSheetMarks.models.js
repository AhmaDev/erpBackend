...
MasterSheetMarks.findById = function (id, result) {
    connection.query(`SELECT * FROM masterSheetMarks WHERE idMasterSheetMarks = ${id}`, (err, res) => {
        if (err) {
...
MasterSheetMarks.update = function (id, data, result) {
    connection.query(`UPDATE masterSheetMarks SET ? WHERE idMasterSheetMarks = ${id}`, data, (err, res) => {
        if (err) {
