...
LessonMark.create = function (newLessonMark, result) {
    connection.query(`INSERT INTO lessonMark SET ?`, newLessonMark, (err, res) => {
        if (err) {
...
LessonMark.findById = function (id, result) {
    connection.query(`SELECT * FROM lessonMark WHERE idLessonMark = ${id}`, (err, res) => {
        if (err) {
...
LessonMark.update = function (id, data, result) {
    connection.query(`UPDATE lessonMark SET ? WHERE idLessonMark = ${id}`, data, (err, res) => {
        if (err) {
...
LessonMark.delete = function (id, result) {
    connection.query(`DELETE FROM lessonMark WHERE idLessonMark = ?`, id, (err, res) => {
        if (err) {
            console.log("Error while deleting LessonMark by ID", err);
            result(err, null);
            return;
        }
        result(null, {message: `LessonMark ID ${id} has been deleted successfully`});
    })
}
