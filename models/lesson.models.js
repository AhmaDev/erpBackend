...
Lesson.getAllYears = function (result) {
  portalConnection.query(
    `SELECT * FROM YearStudy WHERE idYearStudy != 18 ORDER BY year DESC`,
    (err, res) => {
      if (err) {
...
Lesson.delete = (id, result) => {
  connection.query(`DELETE FROM lesson WHERE idLesson = ${id}`, (err, res) => {
    ...
    connection.query(
      `DELETE FROM lessonMark WHERE lessonId = ${id}`,
      (err, res) => {
        if (err) {
          console.log("Error while deleting a lesson", err);
          result(err, null);
        ...
        connection.query(
          `DELETE FROM masterSheetMarks WHERE lessonId = ${id}`,
          (err, res) => {
            if (err) {
              console.log("Error while deleting a lesson", err);
              result(err, null);
            ...
            result(null, {
              message: `Lesson ID ${id} has been deleted successfully`,
            });
          },
        );
      },
